    ...
    async function fetchSeries() {
      ...
      if (error) {
        ...
        const transformedSeries: Series[] = data.map((s) => {
          ...
          const seriesPosts = (s.series_posts ?? []) as JoinedSeriesPost[];
          const posts = seriesPosts
            .map((sp) => sp.posts)
            .filter((p): p is JoinedPost => p !== null);

          // Sort by order_column if available, otherwise by published_at
          const sortedPosts = posts.sort((a, b) => {
            // Check if we have order_column from series_posts
            const aOrder = seriesPosts.find(
              (sp) => sp.posts?.id === a.id,
            )?.order_column;
            const bOrder = seriesPosts.find(
              (sp) => sp.posts?.id === b.id,
            )?.order_column;
          ...
          const topPosts = sortedPosts.slice(0, 2).map((post) => ({
            id: post.id,
            title: post.title,
            slug: post.slug,
